...
class Sinkhorn():
    ...
    def fit_transform(self, X, verbose=True, report_interval=500, X_true=None):
        ...
        imps_mean = imps.clone()
        imps.requires_grad = True
        imps_changes = {idx: [] for idx in range(imps.numel())}
        for idx, value in enumerate(imps.view(-1)):  # Flatten `imps` for easy indexing
                imps_changes[idx].append(value.item())
        X_filled_mean= X.detach().clone()
        X_filled_mean[mask.bool()] = imps
        ...
        if X_true is not None:
            return X_filled, maes, rmses, imps_mean, imps, loss_history, X_filled_mean,sinkhorn_loss_history
        else:
            return X_filled, imps_mean, imps,  loss_history, X_filled_mean, sinkhorn_loss_history

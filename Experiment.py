...
def sinkhor(groundTruth_tensor, miss_data_tensor, mask, niter, bucket_specs, X_cols, Y_cols, Z_cols):
    ...
    sk_imp_data, sk_mae_track, sk_rmse_track = sk_imputer.fit_transform(miss_data_tensor, verbose=True,
                                                                        report_interval=50, X_true=groundTruth_tensor)
...
# sinkhorn_CMI
def sinkhornCMI(groundTruth_tensor, miss_data_tensor, mask, niter, highest_lamda_cmi, bucket_specs, X_cols, Y_cols,
                Z_cols):
    ...
    sk_cmi_imputer = SinkhornImputation_CMI(eps=epsilon, batchsize=batchsize, lr=lr, niter=niter,
                                            highest_lamda_cmi=highest_lamda_cmi)
    (sk_cmi_imp, sk_cmi_mae_track, sk_cmi_rmse_track, cmi_loss_track, sinkhorn_loss_track, lamda_cmi_track)\
        = sk_cmi_imputer.fit_transform(
        miss_data_tensor,
        True,
        50,
        groundTruth_tensor,
...
def run(groundTruth_tensor, X, Y, cycle, missing_type, missing_rate, dependencies, highest_lamda_cmi, niter,
        bucket_specs, X_cols, Y_cols, Z_cols):
    sk_mae = []
    sk_cmi = []
    skCmi_mae = []
    skCmi_cmi = []
    ...
    for i in range(cycle):
        miss_data_tensor, index_mnar, mask = generateMissingValues(X, Y, missing_type, missing_rate,
        ...
        sk_imp_data, sk_mae_track, sk_imp_mae, sk_imp_cmi = sinkhor(groundTruth_tensor, miss_data_tensor, mask, niter,
                                                                    bucket_specs, X_cols, Y_cols, Z_cols)
        sk_mae.append(sk_imp_mae.detach())
        sk_cmi.append(sk_imp_cmi.detach())

        sk_cmi_imp, sk_cmi_mae_track, sk_cmi_rmse_track, cmi_loss_track, sinkhorn_loss_track, lamda_cmi_track, skCmi_imp_mae, skCmi_imp_cmi = sinkhornCMI(
            groundTruth_tensor, miss_data_tensor, mask, niter, highest_lamda_cmi, bucket_specs, X_cols, Y_cols, Z_cols)
        skCmi_mae.append(skCmi_imp_mae.detach())
        skCmi_cmi.append(skCmi_imp_cmi.detach())

        mean_imp_data_tensor, mean_imp_mae, mean_imp_cmi = mean(groundTruth_tensor, miss_data_tensor, mask,
                                                                bucket_specs, X_cols, Y_cols, Z_cols)
        mean_mae.append(mean_imp_mae.detach())
        mean_cmi.append(mean_imp_cmi.detach())

        ice_imp_data_tensor, ice_imp_mae, ice_imp_cmi = ice(groundTruth_tensor, miss_data_tensor, niter, mask,
                                                            bucket_specs, X_cols, Y_cols, Z_cols)
        ice_mae.append(ice_imp_mae.detach())
        ice_cmi.append(ice_imp_cmi.detach())

        soft_imp_data_tensor, soft_imp_mae, soft_imp_cmi = softImpute(groundTruth_tensor, miss_data_tensor, mask,
                                                                      bucket_specs, X_cols, Y_cols, Z_cols)
